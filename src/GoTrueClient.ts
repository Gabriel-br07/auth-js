...
export default class GoTrueClient {
    ...
    if ("message" in credentials) {
      ...
      if ("signIn" in resolvedWallet && resolvedWallet.signIn) {
        ...
        message = [
          ...
          `URI: ${url.href}`,
          `Issued At: ${
            options?.signInWithSolana?.issuedAt ?? new Date().toISOString()
          }`,
   ...
   */
  private _isImplicitGrantCallback(params: {
   ...
   */
  private async _isPKCECallback(params: {
    [parameter: string]: string;
  }): Promise<boolean> {
    ...
    try {
      ...
      if (currentSession && this.userStorage) {
        ...
        if (!currentSession.user) {
          ...
          const separateUser: { user: User | null } | null =
            (await getItemAsync(
              this.storage,
              this.storageKey + "-user"
            )) as any;
      ...
      const expiresWithMargin =
        (currentSession.expires_at ?? Infinity) * 1000 - Date.now() <
        EXPIRY_MARGIN_MS;
      ...
      this._debug(
        ...
        } expired with margin of ${EXPIRY_MARGIN_MS}s`
      );
  ...
  private async _saveSession(session: Session) {
    ...
    const userIsProxy =
      ...
      (sessionToProcess.user as any).__isUserNotAvailableProxy === true;
    if (this.userStorage) {
      ...
      const mainSessionData: Omit<Session, "user"> & { user?: User } = {
        ...sessionToProcess,
      };
