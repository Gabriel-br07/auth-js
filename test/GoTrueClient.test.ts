  ...
  describe("Sessions", () => {
    ...
    test("_callRefreshToken() should resolve all pending refresh requests and reset deferred upon AuthError", async () => {
      ...
      const { session: session3, error: error3 } =
        await authWithSession._callRefreshToken(data.session!.refresh_token);
...
describe("User management", () => {
  ...
  test("refreshAccessToken()", async () => {
    ...
    const { error, data: refreshedSession } =
      await authWithSession._refreshAccessToken(
  ...
  cases.forEach((c) => {
    test(`${c.desc}`, async () => {
      ...
      authWithAsymmetricSession["fetch"] = async (
        ...
        return new Response(
          JSON.stringify({
            keys: [{ kid: "123", kty: "RSA", key_ops: ["verify"] }],
          })
        );
      };
  ...
  describe("_recoverAndRefresh", () => {
    test("should recover and refresh session when valid session exists", async () => {
      ...
      const session: Session | null =
        await autoRefreshClient._recoverAndRefresh();
