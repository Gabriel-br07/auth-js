...
describe("navigatorLock", () => {
  ...
  it("should acquire and release lock successfully", async () => {
    ...
    (globalThis.navigator.locks.request as jest.Mock).mockImplementation(
      (_, __, callback) => Promise.resolve(callback(mockLock))
    );
  ...
  it("should handle immediate acquisition failure", async () => {
    (globalThis.navigator.locks.request as jest.Mock).mockImplementation(
      (_, __, callback) => Promise.resolve(callback(null))
    );

    await expect(
      navigatorLock("test", 0, async () => "success")
    ).rejects.toThrow();
  });

  it("should not throw if browser is not following the Navigator LockManager spec", async () => {
    await expect(
      navigatorLock("test-lock", 1, async () => "success")
  ...
  it("should serialize access correctly", async () => {
    ...
    for (let i = 0; i <= 1000; i += 1) {
      const acquireTimeout =
        Math.random() < 0.3 ? Math.ceil(10 + Math.random() * 100) : -1;
    ...
    expect(Math.ceil((end - start) / timestamps.length)).toBeGreaterThanOrEqual(
      10
    );
