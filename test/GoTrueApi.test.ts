...
describe("GoTrueAdminApi", () => {
  describe("User creation", () => {
    ...
    test("createUser() with user metadata", async () => {
      ...
      expect(data.user?.user_metadata?.profile_image).toMatch(
        /https.*avatars.*(jpg|png)/
      );
    });
  ...
  describe("List users", () => {
    test("listUsers() should return registered users", async () => {
      const { email } = mockUserCredentials();
      const { error: createError, data: createdUser } =
        await createNewUserWithEmail({ email });
      expect(createError).toBeNull();
      expect(createdUser.user).not.toBeUndefined();

      const { error: listUserError, data: userList } =
        await serviceRoleApiClient.listUsers();
  ...
  describe("Get user", () => {
    ...
    test("getUserById() should a registered user given its user identifier", async () => {
      ...
      const { error: foundError, data: foundUser } =
        await serviceRoleApiClient.getUserById(uid);
    ...
    test("getUserById() returns AuthError when user id is invalid", async () => {
      const { error, data } = await serviceRoleApiClient.getUserById(
  ...
  describe("User updates", () => {
    test("modify email using updateUserById()", async () => {
      ...
      const { error: updatedError, data: updatedUser } =
        await serviceRoleApiClient.updateUserById(uid, attributes);
  ...
  describe("User deletes", () => {
    test("deleteUser() should be able delete an existing user", async () => {
      ...
      const { error: deletedError, data: deletedUser } =
        await serviceRoleApiClient.deleteUser(uid);

      expect(deletedError).toBeNull();
      expect(deletedError).not.toBeUndefined();
      expect(deletedUser.user).not.toBeUndefined();

      const { error: listUserError, data } =
        await serviceRoleApiClient.listUsers();
      expect(listUserError).toBeNull();
    ...
    test("deleteUser() returns AuthError when user id is invalid", async () => {
      const { error, data } = await serviceRoleApiClient.deleteUser(
        NON_EXISTANT_USER_ID
      );
  ...
  describe("User registration", () => {
    test("generateLink supports signUp with generate confirmation signup link", async () => {
      ...
      expect(properties?.["action_link"]).toMatch(
        new RegExp(`redirect_to=${redirectTo}`)
      );
    });
    ...
    test("inviteUserByEmail() creates a new user with an invited_at timestamp", async () => {
      ...
      const redirectTo = "http://localhost:9999/welcome";
      const userMetadata = { status: "alpha" };
      const { error, data } = await serviceRoleApiClient.inviteUserByEmail(
        email,
        {
          data: userMetadata,
          redirectTo,
  ...
  describe("User authentication", () => {
    describe("signOut()", () => {
      test("signOut() with an valid access token", async () => {
        ...
        const { error } = await serviceRoleApiClient.signOut(
  ...
  describe("Update User", () => {
    test("updateUserById() returns AuthError when user id is invalid", async () => {
      const { error, data } = await serviceRoleApiClient.updateUserById(
        NON_EXISTANT_USER_ID,
        {
          email: "new@email.com",
        }
  ...
  describe("MFA Admin", () => {
    test("mfa factor management: add, list and delete", async () => {
      ...
      const { data: latestData, error: latestError } =
        await serviceRoleApiClient.mfa.listFactors({
          userId: uid,
        });
